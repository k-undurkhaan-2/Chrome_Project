...
local function assert_mvp0_loaded()
  if type(MVP0) ~= 'table' or type(MVP0.make_input) ~= 'function' or type(MVP0.run) ~= 'function' or type(MVP0.score_candidate) ~= 'function' then
  ...
  if wanted >= total then
    ...
    for slot = 1, wanted do
      ...
      if used[index] == nil then
        used[index] = true
        table.insert(addresses, to_address_number(foundlist.Address[index]))
      end
    end
  end
...
local function select_top_scored(addresses, wanted_count, target_value_pattern)
  if wanted_count == nil or wanted_count <= 0 or #addresses <= wanted_count then
  ...
  local selected = {}
  for i = 1, math.min(wanted_count, #scored) do
    table.insert(selected, scored[i].addr)
  end
...
function MVP0FoundList.collect(opts)
  ...
  if use_prescore_selection and #sorted_addresses > 0 then
    selected_addresses, prescored_count = select_top_scored(sorted_addresses, max_candidates, target_value_pattern)
    selection_strategy = 'prescore_top_scored'
  else
    selected_addresses = select_evenly(sorted_addresses, max_candidates)
